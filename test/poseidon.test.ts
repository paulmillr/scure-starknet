...
function parseTest(path) {
  let data = readFileSync(joinPath(_dirname, 'vectors', 'poseidon', path), 'ascii');
  // Remove whitespaces
  data = data.replace(/[ |\t]/g, '');
  const pattern =
    'Rate=(\\d+)\n' +
...
const parsed = {
  poseidon3: parseTest('poseidon3.txt'),
  poseidon4: parseTest('poseidon4.txt'),
  poseidon5: parseTest('poseidon5.txt'),
  poseidon9: parseTest('poseidon9.txt'),
};
...
should('Poseidon examples', () => {
  ...
  eql(p9([0n, 0n, 0n, 0n, 0n, 0n, 0n, 0n, 0n]), [
...
should('Poseidon 2', () => {
  ...
  // poseidon_hash_func
  eql(
    bytesToHex(starknet.poseidonHashFunc(new Uint8Array([1, 2]), new Uint8Array([3, 4]))),

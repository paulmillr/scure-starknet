...
should('RFC6979', () => {
  for (const msg of sigVec.messages) {
    const sig = starknet.sign(msg.hash, sigVec.private_key);
...
should('Signatures', () => {
  ...
  for (const v of vectors) {
    ...
    const { r, s } = starknet.Signature.fromHex(sig);
    deepStrictEqual(r.toString(16), v.r);
    deepStrictEqual(s.toString(16), v.s);
    deepStrictEqual(starknet.verify(sig, v.msg, publicKey), true);
  }
});
